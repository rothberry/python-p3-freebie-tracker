...
- Create an ORM to map our Classes to the database.
...
You will need to create the `freebies` table using the
...
- A `Freebie` belongs to a `Dev`, and a `Freebie` also belongs to a `Company`.
  In your `__init__`, create any columns (attributes) your `freebies` table will need to
...
After creating the `freebies` table, use the `seed.py` file to
...
For each Model in our database, we will need to create the `__init__` method that sets all the ***properties*** of the methods.

You will also need to provide `setters` and `getters` with *"private"* attributes. For all of the `setter` methods, you will also need to flag an `Exception` if we try to pass in incorrect data.

#### Company
- `Company __init__(name)`
...
- `Company property name()`
  ...
  - must be a string of greater than zero characters
    - If not, you need to return an `Exception`, for Example
